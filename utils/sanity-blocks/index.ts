...
const serializerIsVueComponent = (serializer: Serializer): serializer is DefineComponent => {
  return (
...
const attachMarks = (
  ...
  const markDef =
    mark in serializers.marks ? { _type: mark, _key: "" } : markDefs.find((m) => m._key === mark)
  ...
  if (serializerIsVueComponent(serializer)) {
    ...
    return h(serializer, props, () => attachMarks(span, marks, serializers, markDefs))
  }
  ...
  return h(serializer, extractProps(markDef), attachMarks(span, marks, serializers, markDefs))
}
...
const linkSerializer: MarkSerializer = (props, children) => {
  return h("a", { href: props.href, target: props.newtab ? "_blank" : undefined }, children)
}
...
const listItemSerializer = (block: BlockListItem, serializers: Serializers) => {
  ...
  return h("li", {}, shouldWrap ? createElementFromStyle(block, serializers, children) : children)
}
...
const nestBlocks = (blocks: Array<Block | BlockSpan>, level = 0) => {
  const isListOrListItem = (block: Block | BlockSpan): block is BlockListItem => "level" in block
  const hasChildren = (block: Block | BlockSpan): block is BlockText | BlockList =>
  ...
  blocks.forEach((block) => {
    ...
    if (block.level && block.level > level) {
      ...
      } else if (lastBlock.level === block.level && lastBlock.listItem !== block.listItem) {
        newBlocks.push(createList(block))
      } else {
...
const renderBlocks = (blocks: Block[] | BlockSpan[], serializers: Serializers, level = 0) => {
  // Nest list items in lists
  const nestedBlocks = nestBlocks(blocks, level)

  // Loop through each block, and serialize it
  return nestedBlocks.map((block) => serializeBlock(block, serializers)).filter(notNull)
}
